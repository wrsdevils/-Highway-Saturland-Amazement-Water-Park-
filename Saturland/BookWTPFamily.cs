...
namespace Saturland
{
    public partial class BookWTPFamily : Form
    {
        public BookWTPFamily()
        {
            InitializeComponent();
        ...
        private void Numeric_Kids_ValueChanged(object sender, EventArgs e)
        {
            ...
            int kids_subtotal = (numkids * 490);
            int kids_discount = (numkids * (490 - 290));
            int kids_total = kids_subtotal - kids_discount;

            int adults_subtotal = (numadults * 690);
            int adults_discount = (numadults * (690 - 490));
        ...
        private void Button_Confirm_Click(object sender, EventArgs e)
        {
            if (TextBox_Fname.Text != "" && TextBox_Lname.Text != "" && dateTimePicker_Visit.Text != "" && ComboBox_Province1.Text != "" && TextBox_Email.Text != "" && TextBox_Telnum.Text != "")
            {
                ...
                {
                    Label_EmailCheck.ForeColor = Color.Blue;
                    Label_EmailCheck.Text = "Correct E-mail";
                }
                else
                {
                    Label_EmailCheck.ForeColor = Color.DarkCyan;
                ...
                if (TextBox_Telnum.Text != "")
                {
                    if (TextBox_Telnum.Text.Length == 10)
                    {
                        ...
                        {
                            Label_TelnumCheck.ForeColor = Color.Blue;
                            Label_TelnumCheck.Text = "Correct Tel. number";
                        }
                        else
                        {
                            Label_TelnumCheck.ForeColor = Color.DarkCyan;
        ...
        private void Button_Really_Click(object sender, EventArgs e)
        {
            ...
            if (dialogResult == DialogResult.Yes)
            {
                string mainconn = ConfigurationManager.ConnectionStrings["Mysqlconnection"].ConnectionString;
                MySqlConnection sqlconn = new MySqlConnection(mainconn);
                MySqlDataAdapter da = new MySqlDataAdapter("SELECT * FROM familyzone WHERE Fname = '" + TextBox_Fname.Text + "' AND Lname = '" + TextBox_Lname.Text + "'", sqlconn);
                ...
                if (TextBox_IDsearch.Text == "")
                {
                    ...
                    else
                    {
                        ...
                        catch (Exception ex)
                        {
                            ...
                            PaymentWTPFamily paymentWTPfamily = new PaymentWTPFamily();
                            paymentWTPfamily.Show();
        ...
        private void Button_View_Click(object sender, EventArgs e)
        {
            ...
            if (ComboBox_SeclectVisa.SelectedItem.ToString() == "Highway Visa Card")
            {
                ...
                if (da.Read())
                {
                    ...
                    double kids_subtotal = (numkids * 490);
                    double kids_discount = (numkids * (490 - 290));
                    double kids_total = kids_subtotal - kids_discount;

                    double adults_subtotal = (numadults * 690);
                    double adults_discount = (numadults * (690 - 490));
                ...
                {
                    int numkids = Convert.ToInt32(Numeric_Kids.Value);
                    int numparents = Convert.ToInt32(Numeric_Adults.Value);

                    double kids_subtotal = (numkids * 490);
                    double kids_discount = (numkids * (490 - 290));
                    double kids_total = kids_subtotal - kids_discount;

                    double adults_subtotal = (numparents * 690);
                    double adults_discount = (numparents * (690 - 490));
                    double adults_total = adults_subtotal - adults_discount;
