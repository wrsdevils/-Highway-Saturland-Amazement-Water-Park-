...
namespace Saturland
{
    public partial class BookAMZKids : Form
    {
        public BookAMZKids()
        {
            InitializeComponent();
        ...
        private void Numeric_Kids_ValueChanged(object sender, EventArgs e)
        {
            ...
            int numparents = Convert.ToInt32(Numeric_Parents.Value);

            int kids_subtotal = (numkids * 580);
            int kids_discount = (numkids * (580 - 280));
            int kids_total = kids_subtotal - kids_discount;

            int parents_subtotal = (numparents * 680);
            int parents_discount = (numparents * (680 - 350));
            int parents_total = parents_subtotal - parents_discount;
            int all_total = kids_total + parents_total;

            Label_Subtotal.Text = (kids_subtotal + parents_subtotal).ToString();
            Label_Discount.Text = "-" + (kids_discount + parents_discount).ToString();

            Label_Total.ForeColor = Color.Khaki;
            Label_Total.Text = (all_total).ToString();
        }

        private void Numeric_Parents_ValueChanged(object sender, EventArgs e)
        {
            int numkids = Convert.ToInt32(Numeric_Kids.Value);
            int numparents = Convert.ToInt32(Numeric_Parents.Value);
        ...
        private void Button_Really_Click(object sender, EventArgs e)
        {
            ...
            if (dialogResult == DialogResult.Yes)
            {
                string mainconn = ConfigurationManager.ConnectionStrings["Mysqlconnection"].ConnectionString;
                MySqlConnection sqlconn = new MySqlConnection(mainconn);
                MySqlDataAdapter da = new MySqlDataAdapter("SELECT * FROM kidsfunny WHERE Fname = '" + TextBox_Fname.Text + "' AND Lname = '" + TextBox_Lname.Text + "'", sqlconn);
                ...
                if (TextBox_IDsearch.Text == "")
                {
                    ...
                    else
                    {
                        ...
                        catch (Exception ex)
                        {
                            ...
                            GlobalPayment.Second = Numeric_Parents.Text;
                            ...
                            PaymentAMZKids paymentAMZKids = new PaymentAMZKids();
                            paymentAMZKids.Show();
                ...
                else
                {
                    ...
                    {
                        sqlconn.Open();
                        MySqlCommand command = new MySqlCommand("INSERT INTO `kidsfunny`(`IDCard`,`Fname`, `Lname`, `Visacard`, `KidsTicket`, `ParentsTicket`, `Email`, `Tel`, `Province`, `Visitdate`, `Pricetotal`, `Bookdate`) VALUES('" + TextBox_IDsearch.Text + "','" + TextBox_Fname.Text + "','" + TextBox_Lname.Text + "','" + ComboBox_SeclectVisa.SelectedItem.ToString() + "', '" + Numeric_Kids.Value + "','" + Numeric_Parents.Value + "','" + TextBox_Email.Text + "','" + TextBox_Telnum.Text + "','" + ComboBox_Province1.Text + "','" + dateTimePicker_Visit.Value.ToString("MMM dd, yyyy") + "','" + Label_Total.Text.ToString() + "','" + DateTime.Now.ToString(("MMM dd, yyyy | h:mm tt ")) + "')", sqlconn);                        
                        command.ExecuteNonQuery();
                        sqlconn.Close();
                        try
                        {
        ...
        private void bunifuFlatButton1_Click(object sender, EventArgs e)
        {
            ...
            if (ComboBox_SeclectVisa.SelectedItem.ToString() == "Highway Visa Card")
            {
                ...
                if (da.Read())
                {
                    ...
                    double kids_subtotal = (numkids * 580);
                    double kids_discount = (numkids * (580 - 280));
                    double kids_total = kids_subtotal - kids_discount;

                    double parents_subtotal = (numparents * 680);
                    double parents_discount = (numparents * (680 - 350));
                    double parents_total = parents_subtotal - parents_discount;
                    ...
                    Label_Subtotal.Text = (kids_subtotal + parents_subtotal).ToString();
                    Label_Discount.Text = "-" + (kids_discount + parents_discount + member_discount).ToString();
