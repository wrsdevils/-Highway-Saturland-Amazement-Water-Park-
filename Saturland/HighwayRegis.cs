...
namespace Saturland
{
    public partial class HighwayRegis : Form
    {
        public HighwayRegis()
        {
            InitializeComponent();
        ...
        private void Button_Really_Click(object sender, EventArgs e)
        {
            ...
            if (dialogResult == DialogResult.Yes)
            {
                string mainconn = ConfigurationManager.ConnectionStrings["Mysqlconnection"].ConnectionString;
                MySqlConnection sqlconn = new MySqlConnection(mainconn);
                MySqlDataAdapter da = new MySqlDataAdapter("SELECT * FROM highwayregis WHERE IDCard = '" + TextBox_IDcard.Text + "'", sqlconn);
